...
<Note>
**Budget and Task Failure Handling**: Budget exceeded and task failures are not exceptions. Check `status.status` for `FAILED` or monitor cost via `status.metrics.cost_usd`.
</Note>
...
Monitor cost and handle failures via status:
...
if status.metrics and status.metrics.cost_usd > 1.00:
    print(f"High cost: ${status.metrics.cost_usd:.2f}")
