...
```python
# Analyze CSV data (structured output)
handle = client.submit_task(
    query=(
        "From the CSV at /data/sales_2024.csv, return the top 3 products by revenue "
        "with month-over-month growth. Output JSON array: [{product, revenue, mom_growth_pct}] sorted desc."
    ),
