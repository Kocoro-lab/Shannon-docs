...
| `AGENT_THINKING` | Agent is reasoning/planning | `"Thinking: What is 5 + 5?"` |
...
<Note>
For a complete list of all 26 event types, see the [Event Types Reference](/en/api/rest/streaming#event-types).
</Note>
...
function TaskMonitor({ taskId, apiKey }) {
  ...
  useEffect(() => {
    ...
    // For authenticated endpoints, pass API key in URL
...
- Browser `EventSource` API doesn't support custom headers, so API keys must be passed as URL query parameters
