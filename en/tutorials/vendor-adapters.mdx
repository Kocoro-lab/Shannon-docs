...
# Usage: SHANNON_CONFIG_PATH=config/overlays/shannon.datainsight.yaml
    ...
    openapi_tools:
      datainsight_analytics:
        enabled: true
        spec_url: file:///app/config/openapi_specs/datainsight_api.yaml
        auth_type: bearer
        auth_config:
          vendor: datainsight  # This triggers adapter loading
          token: "${DATAINSIGHT_API_TOKEN}"
          # extra_headers supports only static and env-resolved values
          # Dynamic header templating from request body is not supported
          extra_headers:
