...
**Core Parameters**:
- `max_iterations` (1–50): Controls main React loop depth
- `max_concurrent_agents` (1–20): Parallel task concurrency limit (ResearchWorkflow currently uses a fixed concurrency of 5)
...
- `report_mode` (bool): Formatting hint. ResearchWorkflow sets `synthesis_style: comprehensive` internally for final reports.
