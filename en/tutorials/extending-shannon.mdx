...
<CardGroup cols={2}>
  ...
  <Card title="Synthesis Templates" icon="file-lines">
...
approvals:
  ...
  timeout_seconds: 1800      # 30 minute approval window
```
...
|--------|-------------------|----------|
| Named template | `synthesis_template: "name"` | Reusable `.tmpl` files |
| Verbatim override | `synthesis_template_override: "..."` | One-time custom format |
| Min length | `synthesis_min_length: N` | Enforce minimum character count |
...
|----------|-------------|
| `_base.tmpl` | Protected base with citation rules |
| `normal_default.tmpl` | Default non-research synthesis (minimal structure) |
| `research_comprehensive.tmpl` | Detailed research format with strong coverage requirements |
| `research_concise.tmpl` | Shorter research synthesis with lighter word requirements |
| `research_with_facts.tmpl` | Research format optimized for fact extraction metadata |
| `test_bullet_summary.tmpl` | Example bullet-point summary template |
...
<Card title="Template Directory" icon="folder">
...
![Extension Decision Tree](assets/extension-decision-tree.svg)
