...
sequenceDiagram
    ...
    Orch->>Agent: Execute Tool (body, operation)
    Agent->>LLM: Tool Request (body)

    alt Vendor Adapter Configured
        LLM->>Adapter: transform_body(body, operation, prompt_params)
        Adapter->>Adapter: Field aliasing<br/>Session injection<br/>Normalization
        Adapter-->>LLM: Transformed body
    end

    LLM->>API: HTTP Request (transformed body)
    API-->>LLM: Response
