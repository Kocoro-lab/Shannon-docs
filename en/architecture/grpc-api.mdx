...
description: "Complete Protocol Buffer definitions for Shannon's internal gRPC services"
---

## Overview

Shannon's internal architecture uses **gRPC** (HTTP/2 + Protocol Buffers) for high-performance inter-service communication. This document provides complete Protocol Buffer definitions for all 5 services and 37 RPC methods.
...
**Services**:
- **OrchestratorService** (18 RPCs) - Task orchestration, workflow management, and scheduling
...
See [Event Types Catalog](/en/api/event-types) for all event types.
...
See [REST API Reference](/en/api/rest/overview) for HTTP endpoints.
