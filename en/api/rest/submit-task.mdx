...
| `model_override` | string | No | Specific model name (e.g., `gpt-4o`, `claude-sonnet-4-5-20250929`) |
...
```json
{
  ...
  "model_override": "gpt-4o",                                   // OPTIONAL: Specific model (supports aliases like "gpt-5" → "gpt-4o")
...
- `model_override` — specific model (e.g., `gpt-4o`, `gpt-4o-mini`, `o1`) — supports aliases
- `provider_override` — force provider (e.g., `openai`, `anthropic`, `google`)
- `template` — template name (alias: `template_name`)
- `template_version` — template version
- `disable_ai` — true to require template only (no AI fallback) — **cannot combine with model controls**
- Advanced context window controls: `history_window_size`, `use_case_preset`, `primers_count`, `recents_count`, `compression_trigger_ratio`, `compression_target_ratio`

Rules:
- Top‑level parameters override context equivalents: `model_tier`, `model_override`, `provider_override`
- `mode` accepts: `simple|standard|complex|supervisor` (default: auto-detect)
- `model_tier` accepts: `small|medium|large`
- **Conflict validation**: `disable_ai: true` cannot combine with `model_tier`, `model_override`, or `provider_override` (returns 400)

Model Aliases:
- `gpt-5` → `gpt-4o`
- `o3-mini` → `gpt-4o-mini`
- Aliases provide user-friendly names and prevent errors
