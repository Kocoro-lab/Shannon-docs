...
| `mode` | string | No | Execution mode hint: `simple` or `supervisor` |
...
```json
{
  ...
  "mode": "simple | supervisor (optional)",
  "model_tier": "small | medium | large (optional)",
  "context": {
    "role": "analysis",
...
- `mode` only accepts `simple|supervisor`.
- `model_tier` only accepts `small|medium|large`.
