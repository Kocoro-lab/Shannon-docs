...
async function submitAndStream(query, onEvent, onComplete, onError) {
  try {
    ...
    eventSource.onmessage = (e) => {
      ...
      if (event.type === 'WORKFLOW_COMPLETED' || event.type === 'TASK_COMPLETED') {
        eventSource.close();
        onComplete(event);
      }
    };
  ...
  const submitTask = useCallback(async (query, sessionId = null) => {
    ...
    try {
      ...
      eventSource.onmessage = (e) => {
        ...
        if (event.type === 'WORKFLOW_COMPLETED' || event.type === 'TASK_COMPLETED' || event.type === 'ERROR_OCCURRED') {
          eventSource.close();
    ...
    with httpx.stream(
        ...
        for line in stream_response.iter_lines():
            if line.startswith("data:"):
                ...
                if event['type'] in ['WORKFLOW_COMPLETED', 'TASK_COMPLETED']:
                    return event
