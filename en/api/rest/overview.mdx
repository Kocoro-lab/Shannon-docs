...
Shannon provides **17 REST endpoints** across 4 categories:
...
| `GET /sessions/{sessionId}/events` | Get session events (excludes LLM_PARTIAL) |
...
<Warning>
Browser `EventSource` doesn't support custom headers. Pass API key as **query parameter**:
</Warning>
...
| Task Execution | 10 minutes (configurable via `WORKFLOW_TIMEOUT_SECONDS`) |
