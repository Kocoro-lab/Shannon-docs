...
![Workflowアーキテクチャ](../assets/workflow-architecture.svg)
...
```python
client.submit_task(
    query="10万ドルの投資で年利8%、5年間のROIを計算してください"
)
```
...
```python
client.submit_task(
    query="スケーラブルなメッセージキューシステムを設計してください。複数のアーキテクチャを評価し、最適なアプローチを推奨してください。"
)
```
...
```python
client.submit_task(
    query="最新のRust非同期ランタイムベンチマークを調べてください"
)
```
...
```python
client.submit_task(
    query="モノリスをマイクロサービスに移行すべきですか？賛成と反対の両方の議論を提供し、推奨事項を示してください。"
)
```
...
![DAG実行](../assets/dag-execution.svg)
...
client.submit_task(
    query="調査：1) GPT-5の機能、2) Claude 3の特徴、3) Geminiの更新"
)
```
...
```python
handle = client.submit_task(query="複雑な分析タスク")
