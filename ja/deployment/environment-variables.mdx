...
<Note>
セマンティックメモリ機能（テキスト埋め込み）に必須です。このキーがない場合、ベクトル検索とエージェントメモリは無効になります。
</Note>
...
**タイプ**: String
**デフォルト**: `google`
**有効な値**: `google`, `serper`, `serpapi`, `searchapi`, `bing`, `exa`, `firecrawl`
**サービス**: LLMサービス
**ホットリロード**: はい
...
**プロバイダー**:
- `searchapi` — SearchAPI.io — マルチエンジン検索 ($0.004/コール)
...
SearchAPI.io の API キー。[searchapi.io](https://www.searchapi.io) で取得できます。
...
**タイプ**: String
**デフォルト**: `/tmp/shannon-sessions`
**サービス**: Gateway, Agent Core
**ホットリロード**: いいえ

セッション分離されたワークスペースファイルのベースディレクトリ。各セッションにサブディレクトリが割り当てられます。
...
ファイル操作のWASIサンドボックスを有効にします。有効にすると、ファイルアクセスはFirecracker実行環境を介してセキュリティ分離されます。
