...
graph TB
    subgraph リクエストフロー
        Request[完了リクエスト] --> KeyGen[キャッシュキー生成]
        KeyGen --> Check{キャッシュヒット?}
        Check -->|はい| Return[キャッシュレスポンス返却]
        Check -->|いいえ| Provider[LLMプロバイダー]
        Provider --> Validate{有効なレスポンス?}
        Validate -->|はい| Store[キャッシュに保存]
        Validate -->|いいえ| Skip[キャッシュスキップ]
        Store --> Return2[レスポンス返却]
        Skip --> Return2
    end

    subgraph キャッシュバックエンド
        Memory[(インメモリLRU)]
        Redis[(Redis)]
    end
