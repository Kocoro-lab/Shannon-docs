...
<Tip>
カスタム変換が必要なドメイン固有のAPIについては、下記の[Vendor Adapter Pattern](#vendor-adapter-pattern)セクションまたは包括的な[Vendor Adapters Guide](/en/tutorials/vendor-adapters)を参照してください。
</Tip>
...
- ✅ 指数バックオフによるリトライロジック（3回のリトライ、`OPENAPI_RETRIES`で設定可能）
- ✅ 設定可能なレート制限とタイムアウト
