...
<Card title="完全なツールガイド" icon="wrench" href="/ja/tutorials/custom-tools">
...
<Card title="完全なベンダーアダプターガイド" icon="plug" href="/ja/tutorials/vendor-adapters">
...
approvals:
  ...
  timeout_seconds: 1800      # 30分の承認ウィンドウ
```
...
![拡張決定木 - 拡張タイプ（テンプレート、MCP/OpenAPIツール、組み込みツール、ベンダーアダプター、カスタム分解）と複雑性レベル（低、中、高）の選択を導く決定木 / Extension Decision Tree - Decision tree guiding selection of extension type (Templates, MCP/OpenAPI Tools, Built-in Tools, Vendor Adapters, Custom Decomposition) and complexity level (Low, Medium, High)](assets/extension-decision-tree.svg)
...
<CardGroup cols={2}>
  ...
  <Card title="ベンダーアダプタ" icon="plug" href="/ja/tutorials/vendor-adapters">
    ドメイン固有の統合を構築
  </Card>
