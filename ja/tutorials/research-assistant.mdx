...
![リサーチワークフロー - メモリ取得、クエリ精錬、分解、実行パターン選択、エンティティフィルタリング、引用収集、ギャップ埋め、合成、検証を含む包括的な研究ワークフローを示すフローチャート / Research Workflow - Flowchart showing comprehensive research workflow including memory retrieval, query refinement, decomposition, execution pattern selection, entity filtering, citation collection, gap filling, synthesis, and verification](assets/research-workflow.svg)
