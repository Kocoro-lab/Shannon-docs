...
| `skill` | string | いいえ | シングルエージェント実行を駆動するMarkdown定義のスキル（[スキルシステム](/ja/api/rest/skills)） |
| `research_strategy` | string | いいえ | リサーチ戦略プリセット: `quick`, `standard`, `deep`, または `academic` |
| `max_iterations` | integer | いいえ | タスクの最大反復回数 (1-50) |
| `max_concurrent_agents` | integer | いいえ | 最大同時実行エージェント数 (1-20) |
| `enable_verification` | boolean | いいえ | タスク完了後に検証ステップを有効にする |
...
- トップレベルのパラメータはコンテキストの同等物を上書きします:
  ...
  - トップレベルの`skill`は`context.skill`を上書きします
  - トップレベルの`research_strategy`は`context.research_strategy`を上書きします
</Warning>
...
- `research_strategy` — _（非推奨: トップレベルの `research_strategy` を使用してください。トップレベルが設定されている場合、contextの値は無視されます）_
- `skill` — _（非推奨: トップレベルの `skill` を使用してください。トップレベルが設定されている場合、contextの値は無視されます）_
...
- **広告リサーチプラットフォームトグル**（`role: "ads_research"`の場合）:
...
役割プリセットは、異なるタスクタイプに対して専門的なシステムプロンプトとツール許可リストを提供します。`context.role`を通じて設定:

| 役割 | 説明 | 利用可能なツール | ユースケース |
|------|------|------------------|--------------|
| `generalist` | 汎用アシスタント（デフォルト） | すべてのツール | 簡単なクエリ、一般的な会話 |
| `analysis` | 構造化された推論を持つ分析アシスタント | `web_search`, `file_read` | データ分析、構造化された推論 |
| `research` | 情報収集と統合のためのリサーチアシスタント | `web_search`, `web_fetch`, `web_subpage_fetch`, `web_crawl` | 情報収集、事実調査 |
| `writer` | 明確で整理された文章のためのテクニカルライター | `file_read` | ドキュメンテーション、コンテンツ作成 |
| `critic` | 欠陥とリスクを特定するための批評的レビュアー | `file_read` | コードレビュー、品質保証 |
| `developer` | ファイルシステムアクセス付き開発者アシスタント | `file_read`, `file_write`, `file_list`, `bash`, `python_executor` | コーディング、デバッグ、ファイル操作 |
| `browser_use` | ブラウザ自動化スペシャリスト | `browser_*` ツール, `web_search` | Webスクレイピング、UI自動化、スクリーンショット撮影 |
| `ads_research` **（Shannon Cloud のみ）** | マルチプラットフォーム広告競合分析 | `ads_serp_extract`, `ads_transparency_search`, `meta_ad_library`, `yahoo_jp_ads` | マーケティングリサーチ、競合広告分析 |
| `financial_news` **（Shannon Cloud のみ）** | 金融ニュースとセンチメント分析 | `news_aggregator`, `alpaca_news`, `sec_filings`, `twitter_sentiment` | 株式ニュース、市場センチメント分析 |
| `data_analytics` | ベンダー統合を備えたデータ分析 | ベンダー固有の分析ツール | ビジネスインテリジェンス、分析レポート |
...
# Meta（Instagram のみ）+ Yahoo Japan
curl -X POST http://localhost:8080/api/v1/tasks \
  -H "X-API-Key: sk_test_123456" \
  -H "Content-Type: application/json" \
  -d '{
    "query": "Instagram と Yahoo Japan でのファッションブランドの広告をリサーチ",
    "context": {
      "role": "ads_research",
      "platforms": {
        "google": false,
...
<Tip>
**プラットフォームのデフォルト**: すべてのプラットフォームがデフォルトで有効です。`platforms`オブジェクトを使用してプラットフォームを選択的に無効にするか、Metaをプラットフォーム別にフィルタリングします（`facebook`, `instagram`, `messenger`, `whatsapp`, `all`）。
</Tip>
...
例については[Unified Submit + Stream](/ja/api/rest/streaming#unified-submit-stream-recommended)を参照してください。
</Tip>

## 関連エンドポイント

<CardGroup cols={2}>
  <Card title="送信 + ストリーム" icon="bolt" href="/ja/api/rest/streaming#unified-submit-stream-recommended">
  ...
  <Card title="タスクの一覧" icon="list" href="/ja/api/rest/list-tasks">
    GET /api/v1/tasks
  </Card>
