...
| `skill` | string | いいえ | シングルエージェント実行を駆動するMarkdown定義のスキル（[スキルシステム](/ja/tutorials/skills)） |
| `research_strategy` | string | いいえ | リサーチ戦略プリセット: `quick`, `standard`, `deep`, または `academic` |
| `max_iterations` | integer | いいえ | タスクの最大反復回数 |
| `max_concurrent_agents` | integer | いいえ | 最大同時実行エージェント数 |
