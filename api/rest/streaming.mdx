...
```
ws://localhost:8080/api/v1/stream/ws?api_key=sk_test_123456
```

Or use header after connection:
```javascript
...
```javascript
const ws = new WebSocket('ws://localhost:8080/api/v1/stream/ws?api_key=sk_test_123456');
...
function connectWebSocket(apiKey) {
  const ws = new WebSocket(
    `ws://localhost:8080/api/v1/stream/ws?api_key=${apiKey}`
  );
...
const ws = connectWebSocket('sk_test_123456');
```
...
<CardGroup cols={2}>
  ...
  <Card title="Get Events" icon="list" href="/api/rest/get-events">
