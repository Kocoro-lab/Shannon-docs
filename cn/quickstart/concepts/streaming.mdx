...
| `AGENT_THINKING` | 智能体正在推理/规划 | `"思考：5 + 5 等于多少？"` |
...
<Note>
有关所有 26 种事件类型的完整列表，请参阅[事件类型参考](/cn/api/rest/streaming#event-types)。
</Note>
...
function TaskMonitor({ taskId, apiKey }) {
  ...
  useEffect(() => {
    ...
    // 对于需要身份验证的端点，在 URL 中传递 API 密钥
    // 注意：浏览器的 EventSource 不支持自定义请求头
    if (apiKey) {
...
- 浏览器的 `EventSource` API 不支持自定义请求头，因此 API 密钥必须作为 URL 查询参数传递
