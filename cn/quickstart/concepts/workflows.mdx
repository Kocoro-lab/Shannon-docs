...
![工作流架构 (Workflow Architecture)](assets/workflow-architecture.svg)
...
![DAG 并行执行 (DAG Parallel Execution)](assets/dag-execution.svg)
...
# 此查询将自动分解为 3 个并行子任务
client.submit_task(
    query="研究：1) GPT-5 能力，2) Claude 3 功能，3) Gemini 更新"
)
```
...
| **UpdateSessionResult** | 持久化会话状态并附加助手消息到会话历史（修复 2025-11-05） |
| **RecordQuery** | 存储到向量记忆 |
...
```bash
# 在 .env
MAX_AGENTS_PER_TASK=5          # 最大并行智能体数（如启用）
MAX_TOKENS_PER_REQUEST=50000   # 每次请求的令牌预算（LLM 服务）
MAX_COST_PER_REQUEST=5.0       # 每次请求的成本限制（美元，LLM 服务）
...
```bash
# 在 .env
AGENT_TIMEOUT_SECONDS=300  # 5 分钟限制（每个代理执行）
```
