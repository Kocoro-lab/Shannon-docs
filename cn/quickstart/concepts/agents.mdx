...
MAX_TOKENS_PER_REQUEST=10000       # 每个任务的令牌预算
MAX_COST_PER_REQUEST=0.50          # 每个任务的成本限制（美元）
REQUEST_TIMEOUT_SECONDS=300        # 任务超时时间
...
<Tip>
Shannon 的智能路由器为每个任务选择最具成本效益的模型，与始终使用高级模型相比，可节省 85-95% 的成本。
</Tip>
