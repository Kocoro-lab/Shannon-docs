...
![智能体生命周期 (Agent Lifecycle)](assets/agent-lifecycle.svg)
...
- **模型选择**：Shannon 根据任务复杂度自动选择模型
  - 小型任务 → `gpt-5-mini`、`claude-haiku`
  - 复杂任务 → `gpt-5`、`claude-opus`
- **上下文管理**：自动管理上下文窗口
- **缓存**：在适用时重用之前的 LLM 响应
...
| 层级 | 模型 | 用例 | 成本 |
|------|--------|----------|------|
| **SMALL** | gpt-5-nano、claude-haiku | 简单查询、高容量 | $ |
| **MEDIUM** | gpt-5-mini、claude-sonnet | 通用目的 | $$ |
| **LARGE** | gpt-5.1、claude-opus | 复杂推理、关键任务 | $$$ |
...
if status.token_usage:
    ...
    print(f"令牌：{status.token_usage.total_tokens}")
```
