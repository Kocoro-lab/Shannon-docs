...
<Note>
导入说明：`from shannon import ConnectionError` 指的是 SDK 的异常类型（不是 Python 内置的 `ConnectionError`）。速率限制请使用 `RateLimitError`，网络问题请使用 `ConnectionError`。退避与断路器等高级模式属于参考实现，请根据生产环境验证与调整。
</Note>
