...
handle = client.submit_task(query="撰写一段约 120 字的 Q4 营收驱动与风险摘要（Markdown），最后一行以‘结论：’开头。")
...
h = client.submit_task("通过 WS 流式输出一段约 120 字的可再生能源政策影响简报。")
...
async def stream_task():
    async with AsyncShannonClient() as client:
        handle = await client.submit_task(query="将下段文字总结为 3 条要点，聚焦营收趋势；输出 Markdown 列表。")
