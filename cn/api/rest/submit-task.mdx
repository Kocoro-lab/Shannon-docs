...
| `model_override` | string | 否 | 指定模型名称（如 `gpt-4o`、`claude-sonnet-4-5-20250929`） |
...
```json
{
  ...
  "model_override": "gpt-4o",                                   // 可选：指定模型（支持别名如 "gpt-5" → "gpt-4o"）
...
- `model_override` — 指定具体模型（如 `gpt-4o`、`gpt-4o-mini`、`o1`）- 支持别名
...
规则：
- 顶层参数会覆盖上下文中的等效参数：`model_tier`、`model_override`、`provider_override`
