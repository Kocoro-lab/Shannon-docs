...
| `skill` | string | 否 | 用于驱动单代理执行的 Markdown 定义技能（[技能系统](/cn/tutorials/skills)） |
...
- `research_strategy` — _（已弃用：请使用顶层 `research_strategy` 字段；context 中的值作为回退）_
- `skill` — _（已弃用：请使用顶层 `skill` 字段；context 中的值作为回退）_
