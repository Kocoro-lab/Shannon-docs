...
<Note>
筛选值使用短格式（如 `RUNNING`）。响应中的 status 字段包含 `TASK_STATUS_` 前缀（如 `TASK_STATUS_RUNNING`）。
</Note>
...
| `total_token_usage` | object | Token使用量和成本指标 |

<Note>
列表响应中 `mode` 字段使用 `EXECUTION_MODE_` 前缀（如 `EXECUTION_MODE_STANDARD`）。单任务 GET 端点返回短格式（如 `standard`）。
</Note>
...
| 字段 | 类型 | 描述 |
|------|------|------|
| `total_tokens` | integer | 使用的总Token数 |
| `cost_usd` | float | 以美元计的总成本 |
| `prompt_tokens` | integer | 提示中的Token数 |
| `completion_tokens` | integer | 完成中的Token数 |
