...
Shannon 通过服务器发送事件(Server-Sent Events, SSE)发出**实时事件**。本文档记录平台**实际发出的 26 种事件类型**、它们的结构以及发生时机。
...
注意：`WORKFLOW_FAILED`、`TASK_COMPLETED`、`TOOL_COMPLETED`、`TOOL_FAILED`、`BUDGET_UPDATE` 等事件不会由流式 API 发出。失败通过 `ERROR_OCCURRED` 表示；完成由 `WORKFLOW_COMPLETED` 表示。
...
**发出时机**: 流式传输期间的增量 LLM 输出块（通常在 UI 中被过滤）
...
13. TOOL_INVOKED / TOOL_OBSERVATION
