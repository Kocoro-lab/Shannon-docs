---
title: "gRPC API 参考"
description: "Shannon 内部 gRPC 服务的完整 Protocol Buffer 定义"
---

## 概述

Shannon 的内部架构使用 **gRPC**（HTTP/2 + Protocol Buffers）实现高性能服务间通信。本文档提供了所有 5 个服务和 37 个 RPC 方法的完整 Protocol Buffer 定义。
...
**服务**：
- **OrchestratorService**（18 个 RPC）- 任务编排、工作流管理和调度
...
参见[事件类型目录](/cn/api/event-types)了解所有事件类型。
...
生成流式 LLM 补全（服务器流式 RPC）。

**请求**：与 `GenerateCompletion` 相同
...
**Gateway REST API** 也可在端口 8080 上使用（gRPC 的 HTTP/REST 包装器）。

参见 [REST API 参考](/cn/api/rest/overview)了解 HTTP 端点。
...
<CardGroup cols={2}>
  <Card title="REST API" icon="globe" href="/cn/api/rest/overview">
    HTTP REST 端点
  </Card>
  <Card title="事件类型" icon="stream" href="/cn/api/event-types">
    流事件目录
  </Card>
  <Card title="Python SDK" icon="python" href="/cn/sdk/python/quickstart">
    Python 客户端库
  </Card>
  <Card title="数据库架构" icon="database" href="/cn/architecture/database-schema">
    数据持久化
  </Card>
</CardGroup>
