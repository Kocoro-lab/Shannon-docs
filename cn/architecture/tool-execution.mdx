...
graph TB
    ...
    subgraph "Agent Core (Rust)"
        D --> E[工具注册表]
        E --> F{工具类型?}
        F -->|内置| G[Python WASI 沙箱]
        F -->|MCP| H[HTTP 客户端]
        F -->|OpenAPI| I[OpenAPI 执行器]
        G --> J[熔断器]
        H --> J
        I --> J
    end
...
```mermaid
graph LR
    A[Agent Core<br/>Rust] --> B[Wasmtime 运行时]
    B --> C[Python 3.11<br/>WASI 解释器]
    C --> D[用户代码]
...
sequenceDiagram
    ...
    Orch->>Agent: 执行工具 (body, operation)
    Agent->>LLM: 工具请求 (body)

    alt 配置了供应商适配器
        LLM->>Adapter: transform_body(body, operation, prompt_params)
        Adapter->>Adapter: 字段别名<br/>会话注入<br/>规范化
        Adapter-->>LLM: 转换后的 body
    end

    LLM->>API: HTTP 请求 (转换后的 body)
    API-->>LLM: 响应
    LLM-->>Agent: 工具结果
    Agent-->>Orch: 结果
```
