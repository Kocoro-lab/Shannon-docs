...
<CardGroup cols={2}>
  ...
  <Card title="综合模板" icon="file-lines">
    自定义研究输出格式
  </Card>
</CardGroup>
...
approvals:
  ...
  timeout_seconds: 1800      # 30 分钟批准窗口
```
...
**用于自定义 Shannon 格式化最终研究答案的方式**

综合模板控制多智能体研究结果的格式化和呈现方式。它们对于深度研究工作流特别有用。
...
| 方法 | 上下文参数 | 用例 |
|--------|-------------------|----------|
| 命名模板 | `synthesis_template: "名称"` | 可重用的 `.tmpl` 文件 |
| 逐字覆盖 | `synthesis_template_override: "..."` | 一次性自定义格式 |
| 最小长度 | `synthesis_min_length: N` | 强制最小字符数 |

### 使用命名模板

在 `config/templates/synthesis/` 中创建模板：
...
curl -X POST http://localhost:8080/api/v1/tasks \
  ...
  -d '{
    ...
    "context": {
      "force_research": true,
      "synthesis_template_override": "你是一个云基础设施分析师。\n\n## 分析格式\n- 从比较表开始\n- 使用 [n] 引用\n- 包含成本分析部分\n- 以建议结束"
    }
  }'
```
...
2. 否则，如果满足以下任一条件：
   ...
   - `context.research_areas` 非空
   → 使用 `research_comprehensive.tmpl`。
...
| 模板 | 描述 |
|----------|-------------|
| `_base.tmpl` | 带引用规则的基础模板（受保护，不建议修改） |
| `normal_default.tmpl` | 默认的非研究综合模板（结构最小化） |
| `research_comprehensive.tmpl` | 详细研究格式，强调全面覆盖 |
| `research_concise.tmpl` | 精简研究格式，字数要求较低 |
| `research_with_facts.tmpl` | 为事实提取元数据优化的研究综合格式 |
| `test_bullet_summary.tmpl` | 示例用的要点摘要模板 |
...
![扩展决策树](assets/extension-decision-tree.svg)
