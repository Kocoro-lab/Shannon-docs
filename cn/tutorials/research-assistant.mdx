...
**核心参数**：
- `max_iterations`（1–50）：控制主 React 循环深度
- `max_concurrent_agents`（1–20）：并行任务的并发上限（ResearchWorkflow 当前固定并发为 5）
- `react_max_iterations`（2–8，默认 5）：按任务 ReAct 循环的迭代深度

**进阶开关**：
- `react_per_task`（布尔）：为每个子任务启用小型 ReAct 循环（复杂度 > 0.7 且 deep/academic 时自动启用）
- `enable_verification`（布尔）：启用与引用对照的论断校验
- `report_mode`（布尔）：格式提示。ResearchWorkflow 会在最终报告内将 `synthesis_style` 设为 `comprehensive`。
