...
**核心参数**：
- `max_iterations`（1–50）：用于触发 react_max_iterations 的前置参数（必须设置以使 react_max_iterations 生效）
...
- `gap_filling_max_gaps`（1–10）：最多检测多少个缺口
- `gap_filling_max_iterations`（1–3）：每个缺口的最大重试次数
