...
<Note>
语义记忆功能（文本嵌入）必需。如果未设置此密钥，向量搜索和 agent 记忆功能将被禁用。
</Note>
...
**类型**: String
**默认值**: `google`
**有效值**: `google`, `serper`, `serpapi`, `searchapi`, `bing`, `exa`, `firecrawl`
**服务**: LLM Service
**热重载**: 是
...
**提供商**:
- `searchapi` — SearchAPI.io — 多引擎搜索 ($0.004/次调用)
...
SearchAPI.io 的 API 密钥。在 [searchapi.io](https://www.searchapi.io) 获取。
...
**类型**: String
**默认值**: `/tmp/shannon-sessions`
**服务**: Gateway, Agent Core
**热重载**: 否

会话隔离工作区文件的基础目录。每个会话会获得一个子目录。
...
启用文件操作的 WASI 沙箱。启用后，文件访问将通过 Firecracker 执行器进行安全隔离。
